...
namespace DatosCURP
{
    public partial class Form1 : Form
    {
        ...
        {
            if (txtAP.Text != string.Empty && txtAM.Text != string.Empty && txtNombre.Text != string.Empty && txtano.Text != string.Empty && txtDia.Text != string.Empty)
            {
                try
                {
                    MessageBox.Show("Tu CURP es 0\n" + cu.generar(txtAP.Text, txtAM.Text, txtNombre.Text, txtano.Text, (cmbmes.SelectedIndex + 1).ToString(), txtDia.Text, cmbSexo.SelectedItem.ToString()));
                }
